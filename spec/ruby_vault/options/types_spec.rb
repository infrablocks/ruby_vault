...
describe RubyVault::Options::Types do
  describe '.flag' do
    it 'builds a flag type' do
      expect(described_class.flag(
               '-flag', RubyVault::Options::Values.boolean(true)
             ))
        .to(eq(RubyVault::Options::Types::Flag.new(
                 '-flag', RubyVault::Options::Values.boolean(true)
               )))
    end
  end
  ...
  describe '.standard' do
    it 'builds a standard type' do
      expect(described_class.standard(
               '-opt',
               RubyVault::Options::Values.string('/some/path')
             ))
        .to(eq(RubyVault::Options::Types::Standard.new(
                 '-opt',
                 RubyVault::Options::Values.string('/some/path')
               )))
    end
