...
describe RubyVault::Options::Values do
  describe '.boolean' do
    it 'builds a boolean value' do
      expect(described_class.boolean('true'))
        .to(eq(RubyVault::Options::Values::Boolean.new('true')))
    end
  end
  ...
  describe '.string' do
    it 'builds a string value' do
      expect(described_class.string('/some/path'))
        .to(eq(RubyVault::Options::Values::String.new('/some/path')))
    end
  end
  ...
  describe '.complex' do
    it 'builds a complex value' do
      expect(described_class.complex([1, 2, 3]))
        .to(eq(RubyVault::Options::Values::Complex.new([1, 2, 3])))
    end
  end
  ...
  describe '.key_value' do
    it 'builds a key value value' do
      expect(described_class.key_value('thing', [1, 2, 3]))
        .to(eq(RubyVault::Options::Values::KeyValue.new(
                 'thing', [1, 2, 3]
               )))
    end
  end
